...
function formatRemainingTime(expiresAt?: string, now = Date.now()) {
  ...
  if (!Number.isFinite(ms) || ms <= 0) return "QR code may have expired";
...
function getFriendlyError(message: string) {
  ...
  if (text.includes("EmailBoundError") || text.includes("bound email")) {
    return "This account has a bound email – UPI extraction not available.";
  }
