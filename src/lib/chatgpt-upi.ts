...
async function resolveCredential(credential: string, proxyUrl = ""): Promise<ResolvedCredential> {
  ...
  if (!cookie) {
    throw new Error("没有识别到有效的 session token / session cookie / session JSON");
  }
  ...
  if (status >= 400 || !isObject(data)) {
    throw new Error(`session token 换取 accessToken 失败（HTTP ${status}）：${compactError(data)}`);
  }
...
async function resolveFreshSessionCredential(credential: string, proxyUrl = ""): Promise<ResolvedCredential> {
  ...
  if (status >= 400 || !isObject(data)) {
    throw new Error(`刷新 session 状态失败（HTTP ${status}）：${compactError(data)}`);
  }
...
export async function validateCredentialForUpiExtraction(credential: string) {
  ...
  for (const attempt of attempts) {
    try {
      const { accessToken, sessionData } = await resolveCredential(credential, attempt.proxyUrl);
      const boundEmail = getBoundEmail(sessionData, accessToken);
      if (boundEmail) throw new EmailBoundError(boundEmail);
  ...
  for (const attempt of attempts) {
    try {
      ...
      if (accountCheck.status >= 400 || !isObject(accountCheck.data)) {
        throw new Error(`accounts/check 订阅检测失败（HTTP ${accountCheck.status}）：${compactError(accountCheck.data)}`);
      }
...
function isNonRetryableCredentialError(error: unknown) {
  ...
  return (
    ...
    message.includes("session 响应中没有 accessToken") ||
    /刷新 session 状态失败（HTTP (401|403)/.test(message) ||
    /session token 换取 accessToken 失败（HTTP (401|403)/.test(message)
  );
}
...
async function extractUpiQrFromCredentialWithProxy(
  ...
  if (checkout.status >= 400 || !isObject(checkout.data) || checkout.data.error) {
    ...
    throw new Error(`创建 UPI checkout 失败：${checkoutError}`);
  }
  ...
  if (init.status >= 400 || !isObject(init.data) || init.data.error) {
    throw new Error(`Stripe custom init 失败：${compactError(init.data)}`);
  }
  ...
  if (Array.isArray(paymentMethods) && paymentMethods.length > 0 && !paymentMethods.map((item) => String(item).toLowerCase()).includes("upi")) {
    throw new Error(`当前 checkout 未下发 UPI，available_payment_method_types=${paymentMethods.join(",")}`);
  }
  ...
  if (confirm.status >= 400 || !isObject(confirm.data) || confirm.data.error) {
    throw new Error(`Stripe UPI confirm 失败：${compactError(confirm.data)}`);
  }
...
export async function extractUpiQrFromCredential(credential: string, options?: UpiExtractionOptions): Promise<ExtractedUpiQr> {
  ...
  if (firstError && errors.length === 0) throw firstError;
  throw new Error(`UPI 二维码生成失败，已尝试 ${attempts.length} 个出口：${errors.join(" | ")}`);
}
